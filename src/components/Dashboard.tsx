import { Play, TrendingUp, Clock, Target, AlertCircle, Loader2, LogOut, User, PlayCircle } from 'lucide-react';
...
export default function Dashboard({ onStart, user, onLogout, onViewPast, onResumeSession }: DashboardProps) {
  const [activeSessions, setActiveSessions] = useState<PastSession[]>([]);
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      if (!user?.uid) return;
      console.log("Fetching past sessions from Firestore...");
      try {
        const q = query(
        ...
        const active: PastSession[] = [];
        const completed: PastSession[] = [];
        
        querySnapshot.forEach((doc) => {
          const session = { id: doc.id, ...doc.data() } as PastSession;
              ...
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
                <div className="space-y-4">
                  {activeSessions.map((session) => (
                    <button key={session.id} onClick={() => onResumeSession(session)} className="flex items-center justify-between p-4 rounded-xl bg-neutral-950/50 border border-neutral-800/50 hover:border-amber-500/30 transition-colors w-full text-left group">
                      <div>
                        ...
                        <div className="text-sm text-neutral-500 flex items-center gap-2">
                          ...
                          <span className="text-amber-500/70 ml-2">• Turn {session.turnCount || 1}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-medium text-amber-400 group-hover:text-amber-300 transition-colors">Resume</div>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                {isLoading ? (
                  ...
                  completedSessions.map((session) => (
                    <button key={session.id} onClick={() => session.evaluation && onViewPast(session.evaluation)} className="flex items-center justify-between p-4 rounded-xl bg-neutral-950/50 border border-neutral-800/50 hover:border-neutral-700 transition-colors w-full text-left">
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <span className="font-medium text-neutral-200">{session.config.mode.replace('_', ' ').toUpperCase()}</span>
                          <span className="text-xs px-2 py-1 rounded-full bg-neutral-800 text-neutral-400">{session.config.theme}</span>
